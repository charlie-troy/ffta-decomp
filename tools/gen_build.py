...
def load_asm_symbols():
    """Addresses of still-assembly functions referenced from decompiled C."""
    path = os.path.join(REPO, "data", "asm_symbols.txt")
    out = {}
...
def main(argv):
    ...
    # Functions that are still assembly but are called from decompiled C need a
    # properly typed Thumb symbol. A linker-script assignment will not do: it
    # has no type, so ld inserts an interworking veneer per call, corrupting
    # the branch and growing the ROM. A `.thumb_func` label placed inside the
    # incbin costs nothing and resolves correctly.
    asm_syms = load_asm_symbols()
    ...
    with open(os.path.join(asm_dir, "rom.s"), "w", newline="\n") as fh:
        ...
        for start, end in gaps:
            ...
            inner = sorted((a, n) for n, a in asm_syms.items()
                           if start <= a - BASE < end)
            pos = start
            for addr, name in inner:
                off = addr - BASE
                if off > pos:
                    fh.write(f'\t.incbin "baserom.gba", {pos:#x}, {off - pos:#x}\n')
                fh.write(f"\t.globl {name}\n\t.thumb_func\n{name}:\n")
                pos = off
            if end > pos:
                fh.write(f'\t.incbin "baserom.gba", {pos:#x}, {end - pos:#x}\n')
            fh.write("\n")

    placed_syms = sum(1 for n, a in asm_syms.items()
                      if any(s <= a - BASE < e for s, e in gaps))
    if asm_syms:
        print(f"asm symbols emitted: {placed_syms} of {len(asm_syms)}")
