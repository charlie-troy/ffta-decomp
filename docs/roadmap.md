...
4. Convert the readable `reference/ai_ability_eval.c` gauntlet into
   ...
   **Current compiler baseline:** the full agbcc candidate and retail are both
   ...
   3,606 / 3,732 comparable bytes (126 mismatches in 53 runs), up from
   ...
   reflect fallthrough, aligning all downstream code. Remaining register
   shaping starts with retail's `r7` signed-effect / `r6` sign flag pair; size
   and CFG equality still do not imply byte identity.
